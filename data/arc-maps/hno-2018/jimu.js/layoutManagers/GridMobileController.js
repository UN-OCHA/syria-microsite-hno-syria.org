...
title:a.label,innerHTML:a.label},h);this.own(l(h,"click",e.hitch(this,function(f){var b;f.stopPropagation();a.inPanel?(b=p.getMarginBox(this.panelContainerNode),c.panel={uri:"themes/DashboardTheme/panels/OnScreenPanel/Panel",position:{relativeTo:"browser",left:b.l,top:b.t,width:b.w,height:b.h}},this.panelManager.showPanel(c).then(e.hitch(this,function(a){this.panels[a.id]=a;this._pushId(a.id);a.setPosition({relativeTo:"browser",left:b.l,top:b.t,width:b.w,height:b.h});0>this.panelOnCloseHandlerIds.indexOf(a.id)&&
(this.own(n.after(a,"onClose",e.hitch(this,function(){this._popId(a.id,!0)}))),this.panelOnCloseHandlerIds.push(a.id));return a})).then(e.hitch(this,function(a){this.panelManager.openPanel(a)}))):(b=p.getMarginBox(h),this._toggleOffPanelWidget(a,b.l+b.w,b.t));d.remove(this.containerNode,"in");d.add(this.containerNode,"out")})))},_toggleOffPanelWidget:function(a,c,d){var f=this.openIds.indexOf(a.id);0<=f?(this.widgetManager.closeWidget(a.id),this.openIds.splice(f,1)):this.widgetManager.loadWidget(a).then(e.hitch(this,
function(b){this.openIds.push(a.id);b.setPosition({left:c,top:d,zIndex:100,relativeTo:"map"});this.widgetManager.openWidget(b);0>this.widgetOnCloseHandlerIds.indexOf(a.id)&&(this.own(n.after(b,"onClose",e.hitch(this,function(){f=this.openIds.indexOf(a.id);0<=f&&this.openIds.splice(f,1)}))),this.widgetOnCloseHandlerIds.push(a.id))}))},setPanelPosition:function(a){k.forEach(this.openPanelIds,e.hitch(this,function(c,d){d===this.openPanelIds.length-1&&(a.zIndex=101);"opened"!==this.panels[c].state&&"active"!==
